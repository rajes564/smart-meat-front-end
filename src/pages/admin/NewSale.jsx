      ...
      {(value.mode === 'UPI' || value.mode === 'CARD') && (
        <div className="flex items-center gap-2 bg-brand-50 border border-brand-200 rounded-xl px-3 py-2">
          ...
          <div className="flex-1">
            <p className="text-xs font-bold text-brand-600">Razorpay will open for UPI payment</p>
            <p className="text-[10px] text-stone-400">GPay · PhonePe · Paytm · Any UPI app</p>
          </div>
          <span className="text-[9px] bg-brand-500 text-white px-1.5 py-0.5 rounded-full font-bold">AUTO</span>
        </div>
      )}
          ...
          {upiVal > 0 && (
            <div className="flex items-center gap-1.5 bg-brand-50 border border-brand-200 rounded-lg px-2.5 py-1.5">
              ...
              <p className="text-[10px] text-brand-600 font-semibold">
                Razorpay will open for UPI portion — ₹{upiVal.toFixed(0)}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-amber-light/40 rounded-xl p-3 border border-amber-200 space-y-1.5">
          ...
          {(value.mode === 'UPI' || value.mode === 'CARD') && parseFloat(value.cashAmt) > 0 && (
            <div className="flex items-center gap-1.5 bg-brand-50 border border-brand-200 rounded-lg px-2.5 py-1.5">
              <span className="text-sm">📱</span>
              <p className="text-[10px] text-brand-600 font-semibold">
                Razorpay will open for ₹{parseFloat(value.cashAmt).toFixed(0)}
              </p>
            </div>
          )}
        </div>
